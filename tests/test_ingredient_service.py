...
def test_find_or_create_ingredient_matches_existing_by_name(session_factory) -> None:
    with session_scope(session_factory) as session:
        existing = Ingredient(name="Karotten", normalized_name="karotten", default_unit="kg")
        session.add(existing)
        session.flush()
        existing_id = existing.id

    with session_scope(session_factory) as session:
        ingredient = ingredient_service.find_or_create_ingredient(session, name="karotten", default_unit="kg")
        assert ingredient.id == existing_id


def test_find_or_create_ingredient_matches_existing_by_alias(session_factory) -> None:
    with session_scope(session_factory) as session:
        existing = Ingredient(name="Karotten", normalized_name="karotten", default_unit="kg")
        session.add(existing)
        session.flush()
        ingredient_service.add_alias(session, existing, "Möhren")
        existing_id = existing.id

    with session_scope(session_factory) as session:
        ingredient = ingredient_service.find_or_create_ingredient(session, name="Möhren")
        assert ingredient.id == existing_id


def test_find_or_create_ingredient_creates_new_when_no_match(session_factory) -> None:
    with session_scope(session_factory) as session:
        assert ingredient_service.find_by_name_or_alias(session, "Müllsäcke 60L") is None
        ingredient = ingredient_service.find_or_create_ingredient(session, name="Müllsäcke 60L", default_unit="Stk")
        assert ingredient.id is not None
        assert ingredient.name == "Müllsäcke 60L"
        created_id = ingredient.id

    with session_scope(session_factory) as session:
        again = ingredient_service.find_or_create_ingredient(session, name="Müllsäcke 60L", default_unit="Stk")
